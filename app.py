...
def start(message):
    ...
    bot.send_message(message.chat.id, 'Привет, дорогой друг, я могу конвертировать валюту!'.format(message.from_user),
                     reply_markup=markup)
...
def bot_message(message):
    if message.chat.type == 'private':
        if message.text == 'Помощь':
            bot.send_message(message.chat.id, 'Для начала конвертации, введите команду в формате: '
                                              '\n--> Название валюты <-- '
            ...
            bot.send_message(message.chat.id, 'Возможности бота ограничены!')
        else:
            try:
